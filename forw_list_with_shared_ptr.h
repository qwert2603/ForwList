...
namespace ForwList {
	...
	public:
		...
		public:
			...
			Node(const T &_t, std::shared_ptr<Node> _n) : value(_t), next(_n) {}
		...
		FL() : head(std::make_shared<Node>(T(), nullptr)) {}
		FL(const FL &_fl) : head(std::make_shared<Node>(T(), copy_from(_fl))) {}
		FL(FL &&_fl) throw() : head(std::make_shared<Node>(T(), move_from(std::move(_fl)))) {}
		FL &operator=(const FL &_fl) {
			if (&_fl != this) {
				std::shared_ptr<Node> new_head_next = copy_from(_fl);
				erase_nodes();
		...
		FL &operator=(FL &&_fl) throw() {
			if (&_fl != this) {
				erase_nodes();
				head->next = move_from(std::move(_fl));
			}
			return *this;
		}
		~FL() = default;
		void reverse() {
			std::shared_ptr<Node> t, a = nullptr, z = head->next;
			while (z) {
				t = z->next;
				z->next = a;
				a = z;
		...
		void max_to_end() {
			...
			std::shared_ptr<Node> m = head->next, p = head->next->next, prev_m = head, prev = head->next;
			while (p) {
				if (p->value > m->value) {
					prev_m = prev;
					m = p;
		...
		void min_to_begin() {
			...
			std::shared_ptr<Node> m = head->next, prev_m = head, p = head->next->next, prev = head->next;
			while (p) {
				if (p->value < m->value) {
					m = p;
					prev_m = prev;
		...
		bool empty() const { return head->next.use_count() == 0; }
		void clear() { head->next.reset(); }
		template<typename F> void sort(F _f) {
			for (std::shared_ptr<Node> n1 = head->next; n1; n1 = n1->next) {
				for (std::shared_ptr<Node> n2 = n1->next; n2; n2 = n2->next) {
	...
	private:
		...
		std::shared_ptr<Node> move_from(FL &&_fl) {
			std::shared_ptr<Node> temp = _fl.head->next;
			_fl.head->next.reset();
			return temp;
		}
		// cкопировать узлы из _fl, вернет указатель на cозданный первый узел 
		// перед этим надо cамоcтоятельно удалить cтарые узлы этого cпиcка
		std::shared_ptr<Node> copy_from(const FL &_fl) {
			std::shared_ptr<Node> temp, result = nullptr;
			if (_fl.head->next) {
				temp = std::make_shared<Node>(_fl.head->next->value, nullptr);
				result = temp;
				for (std::shared_ptr<Node> f = _fl.head->next->next; f; f = f->next) {
					temp = (temp->next = std::make_shared<Node>(f->value, nullptr));
				}
			}
			return result;
		}
		...
		void insert_after_node(std::shared_ptr<Node> _n, const T&_v) {
			if (!_n) {
				throw std::runtime_error("! can't insert node after end");
			}
			_n->next = std::make_shared<Node>(_v, _n->next);
		}
		// удалить узел поcле _n
		void erase_after_node(std::shared_ptr<Node> _n) {
			if (!_n) {
	...
	public:
		explicit FLPtr(std::shared_ptr<typename FL<T>::Node> _n = nullptr) : ptr(_n) {}
		T &operator*() {
			if (!ptr) {
	...
	private:
		std::shared_ptr<typename FL<T>::Node> ptr;
	};
	...
	template<typename T> class ConstFLPtr {
		...
		friend bool operator==<T>(const FLPtr<T> &_i1, const ConstFLPtr<T> &_i2);
	public:
		explicit ConstFLPtr(std::shared_ptr<typename FL<T>::Node> _n = nullptr) : ptr(_n) {}
